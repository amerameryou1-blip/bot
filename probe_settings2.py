#!/usr/bin/env python3
"""Dismiss privacy popup -> Game Menu -> Settings -> find color option."""
...
with sync_playwright() as p:
    ...
    # dismiss any privacy popup: look for Accept button
    els = dump_els(page, "initial")
    acc = next((e for e in els if 'Accept' in e['text']), None)
    if acc:
        page.mouse.click(acc['x'], acc['y'])
        log("accepted privacy")
        time.sleep(2)
    ...
    if set_btn:
        page.mouse.click(set_btn['x'], set_btn['y'])
        log(f"clicked Settings at ({set_btn['x']},{set_btn['y']})")
        time.sleep(3)
    dump_els(page, "settings")
    page.screenshot(path=f"{OUT}/settings2.png")
    txt = pytesseract.image_to_string(Image.open(f"{OUT}/settings2.png"), config="--psm 6").strip()
    log("OCR settings:\n" + txt[:800])
    browser.close()
